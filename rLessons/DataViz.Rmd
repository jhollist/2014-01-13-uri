...
```{r eval=FALSE}
set.seed(45)
x<-c(c(1:40) + rnorm(40,sd=10),
     c(41:60) + rnorm(20,sd=10),
     c(61:100) + rnorm (40, sd=10))
...
```{r eval=FALSE}
boxplot(df)
# Not very interesting since these all have the same distribution
# Can use factors and formula calls (haven't go on over these)
# To get more inf
...
```{r eval=FALSE}
# aes() are the "aesthetics" info.  I find this confusing.
# as you are mostly just passing the variables to plot
...
mdf<-melt(df)
ggplot(mdf,aes(x=factor(variable),y=mean(value+c(0,10,100))))+
  geom_bar()
```
